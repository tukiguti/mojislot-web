...
<img src="docs/game-flow.png" width="560" alt="ゲームフロー。ホールで台を選び、通常時に演出を読んで目押しし、BIG/REGを揃えてボーナスへ。ボーナス中はコンボを繋ぎ、再当選を揃えてゲーム数を上乗せする。終了後は通常時へ戻る">

> 図は `python3 tools/gen_game_flow.py` で生成する。**数値を直す場所はスクリプト内の `SPEC` だけ。**

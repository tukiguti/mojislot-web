...
通常時（ほぼ増えない＝実機のコイン持ち）
  ...
  ├─ 示唆   … 候補役を並べて「どれかな…？」。第1・第2停止で当たり役が中段に来たら「狙え！」へ発展
  ├─ 狙え！ … 予告された3文字を狙う
...
| 規定 | 3枚掛け |
| 内部役 | **13種**（ハズレ／1枚役／押し順役5／小役2／チェリー／REG／BIG×2） |
| 示唆tier | **青/緑（小役）・赤（RB）・金（BB）**。候補提示 →「狙え！」へ発展（発展率 約72%） |
| **BIG** | 18G＋α ／ 平均 **113〜241枚**（腕による） |
| **REG** | 8G＋α ／ 平均 **51〜106枚** |
| 合算突入率 | **1/79 〜 1/113**（腕による）。うち確定告知ランプ 23〜33%・チェリー重複 16〜20% |
| ゲーム数上乗せ（おかわり） | **上限なし** |
| 通常時の純増 | **−0.90 〜 +0.65枚/G**（＝ほぼ増えない。実機のコイン持ちに相当） |
| ボーナス中の純増 | **+3.2 〜 +10.0枚/G**（出玉はここに集約） |
| **機械割** | **84% 〜 166%** ─ 設定ではなく、**腕（目押し＋押し順＋読み）で変動** |
...
- **1枚役**：実機の制御用1枚役に相当する**実在の並び**（各章2〜3種・既存文字の「単語にならない」3文字）。当選時は自動で引き込まれて1枚。窓外ならこぼれる（引き込みカバー率 約76〜90%）。押し順ミスのこぼし先もこれ

...
| **BIG** | 18G＋α ／ 平均 **121〜238枚**（腕による） |
| **REG** | 8G＋α ／ 平均 **55〜102枚** |
| 合算突入率 | **1/84 〜 1/115**（腕による）。うち確定告知ランプ 23〜35%・チェリー重複 13〜18% |
| ゲーム数上乗せ（おかわり） | **上限なし** |
| 通常時の純増 | **−0.75 〜 +0.71枚/G**（＝ほぼ増えない。実機のコイン持ちに相当） |
| ボーナス中の純増 | **+3.7 〜 +9.6枚/G**（出玉はここに集約） |
| **機械割** | **89% 〜 163%** ─ 設定ではなく、**腕（目押し＋読み）で変動** |
...
- **順押し前提**：押し順は「役の種類」ではなく**停止制御の入力**。プレイヤーは左→中→右で打てばよく、押し順を覚える必要は無い（押し順役・押し順ナビは2026-07-25に廃止）

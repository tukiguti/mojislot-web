...
| **REG** | 8G＋α ／ 平均 **54〜121枚** |
| 合算突入率 | **1/96 〜 1/104**（持ち越しがあるので腕ではほぼ変わらない。この幅は設定差）。うち確定告知ランプ 27〜30%・チェリー重複 13〜22% |
| ゲーム数上乗せ（おかわり） | **上限なし** |
| 通常時の純増 | **−1.08 〜 +0.74枚/G**（＝ほぼ増えない。実機のコイン持ちに相当） |
| ボーナス中の純増 | **+3.6 〜 +11.7枚/G**（出玉はここに集約） |
| **機械割** | **88% 〜 188%** ─ 動かすのは**腕（目押し＋出目の読み）が主、設定が従**。同じ設定でも初心者88%と神188%で約72ポイント開くのに対し、設定1〜6の幅は中級で13ポイント |
| **設定 1〜6** | 台ごとに日替わりで決まり、プレイヤーには見えない。差は**演出の出方**に乗せてある（高設定ほど無演出が減る＝何を狙えばよいか分かるゲームが増える＝取りこぼしが減る）。ボーナス確率にも小さく差がある |
| ボーナス持ち越し | こぼしても**フラグは消えない**（実機Aタイプ）。以降は無告知で、**リーチ目**を読んで自力で揃える |
| リーチ目 | **第1停止**の中段に**ボーナス役にしか使わない図柄**が出たらボーナス確定。停止テーブルが非ボーナス時はそこに止めないため誤告知は起きない（持ち越し中の61〜96%で出現） |
...
- **台を選ぶところからゲーム**：章のカードを選ぶのではなく、**ホールの24台（6島×4台）から1台を選ぶ**。入口 → 島 → 台に寄る、の3段階。同じ島の4台は**リール配列が同じで違うのは設定だけ**なので、覚えた配列は島の中で持ち越せる（台移動しても技術が無駄にならない）
- **設定推測**：設定1〜6は日付と台番号から決定的に決まり、打っている間は変わらず、日替わりで変わる。プレイヤーは直接見られず、**データカウンター（BIG/REG/ハマり/スランプグラフ）とボーナス終了画面の示唆、そして「今日はやたら演出が出る」という体感**から読む。ホールの方針（末尾・島・角・全台系・通常営業）が先に決まり、そこから各台の設定とポスターが決まるので、朝イチでも読む取っ掛かりがある
- **設定差は演出の出方に乗る**：このゲームの演出は**情報だけ**を持つ（ガセ無し・引き込み窓は演出で変わらない）ので、「無演出が減る → 何を狙えばよいか分かるゲームが増える → 取りこぼしが減る」が直接つながる。結果として**腕がある人ほど設定差が大きく出る**（中級13ポイント / 神28ポイント）。演出の**種類の比**は設定で変えない（「クイズが多い＝高設定」という別の読み筋を作らないため）
...
- **リーチ目と持ち越し**：ボーナスをこぼしても**フラグは消えず**（実機Aタイプ）、以降は演出も告知も出ない。代わりに**リーチ目**が出る——各リールには「ボーナス役にしか使わない図柄」があり、停止テーブルが**非ボーナス時は第1停止でそれを中段に止めない**よう組んであるので、**第1停止の中段に出た＝ボーナス確定**になる（実機の「左中段◯◯」と同じ）。持ち越し中の61〜96%で出現し、読めた人はそこから自力で揃えに行く

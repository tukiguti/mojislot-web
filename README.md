...
| **BIG** | 18G＋α ／ 平均 **119〜306枚**（腕による） |
| **REG** | 8G＋α ／ 平均 **54〜134枚** |
| 合算突入率 | **1/79 〜 1/142**（腕による） |
| ゲーム数上乗せ（おかわり） | **上限なし** |
| 通常時の純増 | **−0.88 〜 +0.78枚/G**（＝ほぼ増えない。実機のコイン持ちに相当） |
| ボーナス中の純増 | **+3.5 〜 +12.7枚/G**（出玉はここに集約） |
| **機械割** | **82% 〜 186%** ─ 設定ではなく、**腕（目押し＋押し順）で変動** |
...
|---|---|---|---|---|---|---|
| 初心者（±100ms / 75%） | **82%** | −0.88枚 | +3.5枚 | 1/142 | 119枚 | 50% |
| 中級（±50ms / 92%） | **110%** | −0.35枚 | +6.0枚 | 1/114 | 166枚 | 67% |
| 上級（±25ms / 100%） | **141%** | +0.15枚 | +8.7枚 | 1/94 | 221枚 | 80% |
| 神（±10ms / 100%） | **186%** | +0.78枚 | +12.7枚 | 1/79 | 306枚 | 91% |
...
- **実機準拠のテーブル制御**：レバーONで内部役（フラグ）を確定し、**当選役以外は4コマ以内で決定的に蹴る**。リール配列もこの制御に合わせて構築的に設計してあり、**「役が揃って見えるのに0枚」は全押下位置で発生しない**（`tests/audit/reel-guarantee.test.ts` が全50万通り×5章で保証）

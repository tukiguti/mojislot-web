 ...
 *
 * 小役は0で、非ボーナスの受け皿は**チェリーだけ**。チェリーには昇格抽選（成立時5%で
 * ボーナス確定）が付いているので、遅れの外れ側もボーナスへ繋がる目が残る。
 * ここを0にすると遅れが「ボーナス確定」になり、示唆の赤・虹と役割が被る。
 */
const DEFAULT_DELAY = {
  ms: 320,
  rate: { core: 0.0, cherry: 0.02, reg: 0.45, big: 0.6 },
};
...
export const TuningSchema = z.object({
   ...
   * 違うので、「引けたら次に期待できる」レア役としての意味づけを与える。
   *
   * 実機の重複当選は内部抽選の時点で同時当選するので、チェリーをこぼしても権利は残る。
   * こちらは成立**後**の抽選なのでこぼすと抽選が起きない。機構が違うため重複とは呼ばない。
   */
  cherryBonus: z
    .object({
      /** チェリー成立1回あたりの当籤率。 */
   ...
   *
   * `spinUpMs` は**リールが定速になるまでの加速時間**（見た目）。短くしてある——
   * ゆっくり見せるとレバーを叩いてから打てるまでがもたつく。ここは待たせる場所
   * ではなく、回り出しに実体を与えるための一瞬。
   *
   * `stopLockMs` は**レバーONから停止ボタンを受け付けるまで**の時間（待ち）。
   * 見た目と待ちを別に振れるように分けてある。
   *
   * **実際のロックは両者の大きい方。** 加速し切っていないリールは物理的に
   * 止められないので、`stopLockMs` を `spinUpMs` より短くしても効かない。
   * 逆に長くすると「定速で回っているのに押せない」状態になる——レバーを塞いだ
   * ときと同じ違和感が出るので、伸ばすなら控えめに。
   *

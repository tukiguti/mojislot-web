...
export const TuningSchema = z.object({
   ...
   *
   * `spinUpMs` は**レバーONからリールが定速になるまで**の加速時間で、
   * 停止ボタンを受け付けるようになるまでの時間でもある（実機と同じく、
   * 定速でないリールは止められない）。レバーやBETを塞ぐのとは別物。
   *
   * **短くしてある。** 加速をゆっくり見せると、レバーを叩いてから打てるまでが
   * もたつく。ここは待たせる場所ではなく、回り出しに実体を与えるための一瞬。

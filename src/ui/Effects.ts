 ...
 * | 1 | 白 | レバーON |
 * | 2 | 青 | 第1停止 |
 * | 3 | 緑 | 第2停止／終了色としては**いちばん弱いが、当たりはする**（次ゲームのボーナス30%・うちBIG22%） |
 * | 4 | 赤 | 次ゲームは**REG寄りで濃厚**（ボーナス70%・うちBIG35%） |
 * | 5 | 金 | 次ゲームは**ボーナス確定でBIG寄り**（ボーナス100%・うちBIG92%） |
 *
 * **緑どまりでも切らない。** 出現率が 緑80% / 赤18% / 金2% なので、**ボーナスの
 * 6割は緑から出ている**。緑を「何も予告しない」にすると、当たりの大半が予告なしで
 * 降ってくることになり、ステップアップを見る意味が消える。率は `main.ts` の
 * `STEP_BONUS_RATE` / `STEP_BIG_RATE`。
 *
